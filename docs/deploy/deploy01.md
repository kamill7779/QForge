...
- 对外暴露地址保持为公网 IP 49.235.113.163
- 服务在 Nacos 中注册公网 IP，而不是 Docker 内部 172.* 地址
- 宿主机增加公网 IP 回环 DNAT，使同机容器访问 49.235.113.163 时自动回流到宿主机私网地址 10.0.0.6
...
把 auth 和 gateway 改成公网地址注册后，跨机器访问方向是对的，但同机容器通过 Nacos 获取到 49.235.113.163:8088 这类地址再回调本机时失败。
...
- 新规则：把公网 IP DNAT 到宿主机私网地址 10.0.0.6

旧规则残留后，会优先生效或与新规则混用，导致 gateway 从容器访问 49.235.113.163:8088 时被错误转发到失效的旧容器地址，出现 connection refused。

### 2.6 Nacos 中存在过期或外部实例

排查过程中还发现 Nacos 中存在额外实例，例如 111.229.17.125:8080。这类实例不是当前节点本地规则能自动消掉的，通常代表另一台机器仍在持续心跳。
...
- auth-service 在 Nacos 中注册为 49.235.113.163:8088
- gateway-service 在 Nacos 中注册为 49.235.113.163:8080
...
```yaml
SPRING_CLOUD_NACOS_DISCOVERY_IP=49.235.113.163
...
- /home/ubuntu/QForge-main/backend/docker-compose.auth-gateway-web.yml
...
- 49.235.113.163:8088 -> 172.19.x.x:8088
...
- gateway-service 运行
- qforge-web-exam 运行
- question-core-service 停止

因此当前登录链路正常，但如果前端继续访问依赖 question-core 的页面或 WebSocket，仍然可能因为下游服务未启用而出现功能缺失。这不属于公网回环或 nginx 问题，而是业务依赖本身未启动。
...
```bash
curl -i http://49.235.113.163:8088/actuator/health
```
...
- auth-service 注册为 49.235.113.163:8088
- gateway-service 注册为 49.235.113.163:8080
...
如果 Nacos 中仍出现例如 111.229.17.125:8080 这类实例，即使手动删除后又重新出现，通常说明有另一台机器仍在持续注册和发送心跳。
...
```bash
cd /home/ubuntu/QForge-main/backend
sudo ./deploy/enable-public-hairpin-nat.sh
```
...
```bash
cd /home/ubuntu/QForge-main/backend
sudo docker compose -f docker-compose.auth-gateway-web.yml up -d --build auth-service gateway-service qforge-web-exam
```
...
3. gateway 容器内访问公网 auth 地址是否返回 200
4. 外部登录接口是否返回业务响应而不是 502/503
...
```bash
sudo iptables -t nat -S | grep '49.235.113.163'
```

### 9.6 查看 gateway 容器到 auth 的公网回流

```bash
sudo docker exec gateway-service sh -lc 'curl -sS --max-time 5 -i http://49.235.113.163:8088/actuator/health | sed -n "1,20p"'
```
...
- auth 和 gateway 显式使用公网地址注册 Nacos
- 统一使用宿主机私网回流的 hairpin NAT
- 删除所有旧的直连容器 DNAT 规则
- 按业务需要决定是否启动 question-core-service
